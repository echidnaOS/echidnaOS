...
void tty_refresh(void) {
    if (current_task->tty == current_tty)
...
static void clear_cursor(void) {
    ...
    if (current_task->tty == current_tty)
        video_mem[(tty[current_task->tty].cursor_offset)+1] = tty[current_task->tty].text_palette;
    return;
}
...
static void draw_cursor(void) {
    if (tty[current_task->tty].cursor_status) {
        ...
        if (current_task->tty == current_tty)
            video_mem[(tty[current_task->tty].cursor_offset)+1] = tty[current_task->tty].cursor_palette;
    }
    return;
}
    ...
    switch (c) {
        ...
        case 0x08:
            if (tty[current_task->tty].cursor_offset) {
                ...
                if (current_task->tty == current_tty)
                    video_mem[tty[current_task->tty].cursor_offset] = ' ';
                draw_cursor();
            }
            break;
        default:
            ...
            if (current_task->tty == current_tty)
                video_mem[tty[current_task->tty].cursor_offset] = c;
